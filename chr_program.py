...
#replace string below with directory of txt file
txt = open("insert directory") 
lines = txt.readlines()
...
while True:
    ...
    new_index = 0
    event_num = 0
    for i in range(0,len(events_round)):
        order = order + "(" + str(event_nums[i]) + ") " + events_round[i] + " " #(1) american revolution
    order = order + "\n"
    print(order)
